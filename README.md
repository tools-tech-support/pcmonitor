...
5. โปรแกรมจะเปิดโฟลเดอร์ `logs` ให้อัตโนมัติ — ไฟล์ที่ต้องส่งไปวิเคราะห์คือ **`session_YYYYMMDD_HHMMSS.txt`** (all-in-one ไฟล์เดียวจบ ไม่ต้องแตก zip: มีสรุปทุกค่า + JSON + timeline รายวินาทีครบในไฟล์เดียว)

ข้าง ๆ กันจะมี `session_YYYYMMDD_HHMMSS.zip` เก็บข้อมูลดิบเผื่อเจาะลึก (ปกติไม่ต้องใช้) ข้างในมี:
